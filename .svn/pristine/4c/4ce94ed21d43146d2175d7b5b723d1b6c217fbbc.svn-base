...
import com.tydic.config.spi.ConfigManager;
import com.tydic.config.spi.Configuration;
import com.tydic.config.spi.Watcher;
...
public abstract class ZKConfigProcess implements ConfigurationProcess{
	...
	public void connect() {
		try {
			zooKeeperOperator.connect(Configuration.HOSTS);
		} catch (IOException e) {
	...
	public String getIndexPath(String confName) {
		...
		if (i != -1) {
			configManager.registerEnv(Configuration.INDEXHOMEENVNAME, classpath[i]);
		}
		return indexPath;
	}
	
	public String getIndex(){
		String envData = configManager.getEnv(Configuration.INDEXHOMEENVNAME);
		Properties props = new Properties();
		String absoluteEnvPath = Configuration.ENV;
		if (envData == null) {
			try {
				...
				props.setProperty(Configuration.INDEXHOMEENVNAME, Configuration.INDEX_HOME);
				writeEnv2ZK(Configuration.INDEXHOMEENVNAME, Configuration.INDEX_HOME);
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			envData = props.getProperty(Configuration.INDEXHOMEENVNAME);
			if (envData == null) {
				envData = Configuration.INDEX;
			}
			configManager.setEnv(props);
		}
		return envData;
	}
	...
	public String[] getDataNameList(String confName, String version, boolean flag, StringBuilder targetIndex){
		...
		if(version==null){
			version = new String(Configuration.DEFAULTVERSION);
		}
		if(targetIndex!=null&&targetIndex.length()>0){
	...
	public void addWatcher(Watcher watcher){
		...
		Configuration conf = configManager.getIndex(path);
		if (conf == null) {
			conf = new Configuration();
			conf.setName(watcher.getConfigName());
			conf.setPath(path);
			StringBuilder tempIndexData = new StringBuilder();
			conf.setData(getConfigAsBytes(conf.getName(),watcher.getVersion(),true, tempIndexData));
			...
			conf.addWatcher(watcher);
			conf.setVersion(watcher.getVersion());
			configManager.registerIndex(conf);
			try {
