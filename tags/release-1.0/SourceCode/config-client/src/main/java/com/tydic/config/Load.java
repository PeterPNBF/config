		...
		if(options.fileName!=null){
			...
			for(String child: children){
				if(child.endsWith(".xml")||child.endsWith(".properties")){
					File file = createFile(options.des + "/" + child.substring(Configuration.INDEX_HOME.length()+1));
					write2File(child.substring(Configuration.INDEX_HOME.length()+1), options.version, file, cp);
				}
			}
		}
		cp.terminate();
